...
for i=1:n+1
    for j=1:n+1
        if(i~=j)
            ...
            p_obj = [p_obj;p_Q(j)*ones(n_arc(i,j),1)];
        end
    end
end
l = length(x_tsp(:,1)); M = 1e6;

A1 = zeros(n,l); A2 = zeros(n+1,l); A3 = zeros(1,l); A4 = zeros(1,l); A5 = zeros(n,l); A6 = zeros(1,l);
ind = 0;
for i=0:n
    for j=0:n
        if(i~=j)
...
sense_mat = char([60*ones(n,1);61*ones(n+2,1);60*ones(n+2,1)]); % 61:'=', 60:'<'

try
    clear model;
    model.A = sparse([A1;A2;A3;A4;A5;A6]);
    model.obj = p_obj;
    model.rhs = [ones(n,1);zeros(n+1,1);1;dv_max;(M-tser)*ones(n,1);25];
...
if(~isfield(result,'pool')), return; end
